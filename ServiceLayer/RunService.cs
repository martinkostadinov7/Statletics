...
namespace ServiceLayer
{
	public class RunService
	{
		...
		public Run GetPersonalBest(int athleteId, int distance)
		{
			List<Run> runs = GetRuns();
			Run run = runs.Where(r => r.AthleteId == athleteId).Where(r => r.Distance == distance)
				.OrderBy(r => r.TimeSeconds)
				.FirstOrDefault();
			if (run == null) throw new ArgumentException("Athlete does not have a personal best in this distance!");
			return run;
		}
	}
}
